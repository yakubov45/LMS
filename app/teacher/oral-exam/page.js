...
export default function Page() {
  ...
  useEffect(() => {
    api.getTeacherOralExams().then((res) => {
      setData(res || []);
      setLoading(false);
    });
  }, []);
  ...
  const handleAddRecord = async () => {
    const res = await api.addTeacherOralExam(formData);
    if (res.success) {
  ...
  return (
    <div className='p-6 md:p-8 lg:p-10 w-full animate-in fade-in duration-500'>
      <div className='flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8'>
        <div>
          <h1 className='text-[40px] font-bold text-[#14213d] tracking-tight'>{t('oralExams')}</h1>
      ...
      {showForm && (
        <Card className='p-8 mb-8 bg-white/80 border border-white rounded-[32px] shadow-xl animate-in zoom-in-95 duration-200'>
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
            <div>
              <label className='text-xs font-black text-slate-500 uppercase tracking-widest mb-2 block'>Description</label>
              <input
            ...
            <table className='w-full text-left min-w-[600px]'>
              <thead>
                <tr className='text-[12px] font-bold text-slate-400 uppercase tracking-wider border-b border-white'>
                  ...
                  <th className='p-5 text-slate-400'>Description</th>
              ...
              <tbody className='divide-y divide-white/50'>
                {data.length === 0 ? (
                  <tr><td colSpan={4} className='py-16 text-center text-slate-400 font-bold text-sm uppercase tracking-widest'>No records yet</td></tr>
                ) : data.map((row, idx) => (
